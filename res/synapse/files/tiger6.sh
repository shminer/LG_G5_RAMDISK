...
case "$1" in
	...
	LiveBWL2Frequencyshow)
		...
		$BB echo "Current L2 frequency：${L2FREQ}@nCurrent bus BW：${BWFREQ}";
	;;
	BWFrequencyList)
	...
	LiveThermalstats)
		...
		if [ ${intelli_enabled} == "Y" ];then
			...
			if [ "${temp}" -gt "${UPT}" ];then
				UT="Upscaling temperature offset：$(expr ${temp} - ${UPT} )ºC";
				else
				UT=`echo "Full Speed！"`;
			fi
			UTS="@n${UT}"
			if [ "${temp}" -ge "${limit_temp_degC}" ];then
				...
				Thermalstats="Triggered thermal control@nLimit temperature difference：${DLST}";
			else
				Thermalstats=`$BB echo "Untriggered thermal@nIntelli Standby"`;
			fi
		else
			Thermalstats=`$BB echo "Intelli inactive"`;
		fi
		if [ ${serveron} == 1 ];then
			THSS=`$BB echo "Thermal-engine active"`;
			else
			THSS=`$BB echo "Thermal-engine Inactive"`;
		fi;
